...
public interface IBoardCallRepository : IRepository<BoardCall, Guid>
{
    ...
    Task<List<BoardCall>> GetActiveCallsForWorkspaceAsync(Guid workspaceId, CancellationToken ct = default);
    ...
    Task<BoardCallParticipant?> GetLatestParticipantAsync(Guid boardCallId, Guid userId, CancellationToken ct = default);
