    ...
    IWorkspaceRepository workspaceRepository) : IWorkspaceAccessService
{
    ...
    {
        Guid userId = await GetCurrentUserIdAsync(ct);

        WorkspaceRole? role = await workspaceRepository.GetUserRoleAsync(workspaceId, userId, ct);
    ...
    {
        WorkspaceRole role = await EnsureIsMemberAsync(workspaceId, ct);
