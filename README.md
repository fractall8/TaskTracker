...
In this project I use a Docker-first approach for our database infrastructure. The database migrations are not executed via EF Core Program.cs. Instead, the Database project is containerized and runs as an Init Container.
...
You can run Docker only for the database and migrations, which is convenient for local debugging:
```bash
docker-compose up -d postgres-db db-migration
```
Before running the API locally, ensure you have the correct ```PostgresConnection``` in your ```appsettings.Development.json``` (or User Secrets) matching the variables in your ```.env``` file, with the host set to ```localhost```:
```json
"ConnectionStrings": {
